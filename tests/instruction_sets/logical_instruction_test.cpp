...
// Helper function for instruction creation
...
TEST(LogicalInstructionTest, OR_PerBit) {
    ...
    // uint32_t result = rf.read(instr.getRs()) | rf.read(instr.getRt());
    // rf.write(instr.getRd(), result);
    execute_logical(instr, rf);
...
TEST(LogicalInstructionTest, OR_WithZeros) {
    ...
    Instruction instr(make_rtype(opcode::OR, 1, 2, 3));
    rf.write(instr.getRd(), rf.read(instr.getRs()) | rf.read(instr.getRt()));
...
TEST(LogicalInstructionTest, XOR_WithZeros) {
    ...
    rf.write(instr.getRt(), rf.read(instr.getRs()) ^ static_cast<uint32_t>(instr.getImmediate()));
...
TEST(LogicalInstructionTest, XOR_WithOnes) {
    ...
    // Ensure immediate is zero-extended for XORI
    uint32_t zero_extended_imm = static_cast<uint16_t>(instr.getImmediate());
    rf.write(instr.getRt(), rf.read(instr.getRs()) ^ zero_extended_imm);

    EXPECT_EQ(
        rf.read(2),
        static_cast<uint32_t>(0x000000AA ^ 0x0000FFFF));  // XORI zero-extends immediate to 32 bits
}
